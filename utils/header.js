/* tools/utils/header.js
   ...
   Flexible config header renderer.
   
   Usage (old API):
     window.renderHeader({ title: 'Tool', toolIconSvg: '...', githubPath: 'tool' })
...
(function () {
  ...
  window.renderHeader = function renderHeader(opts) {
    ...
    const ghPath = config.githubPath;
    const ghHref = config.showGithub && ghPath
      ? `https://github.com/humor4fun/tools/tree/main/${ghPath}`
      : null;

    header.innerHTML = `
      ${config.showLogo ? `<a id="hdr-logo" href="/" aria-label="tools home">
        ${SQUARES_SVG}
        tools
      </a>
      ...
      ${config.showTabs ? '<div id="tab-bar"></div>' : ''}
